...
// Derivative paths and their dimensions. The generator resizes to fill, so the
// SHORTER side is the target and the longer one scales with it. Dimensions are
// computed rather than measured because measuring 82 files would mean opening
// them on every build, and the aspect ratio is all an <img> needs to hold its
// box.
const DERIVATIVES = [
  { key: "thumb", dir: "t184", edge: 184 },
  { key: "card", dir: "t640", edge: 640 },
];

function derivatives(file, width, height) {
  const out = {};
  const shortest = Math.min(width || 0, height || 0);
  for (const size of DERIVATIVES) {
    const scale = shortest ? size.edge / shortest : 1;
    out[size.key] = `/images/photo/${size.dir}/${file}`;
    out[`${size.key}Width`] = Math.round((width || size.edge) * scale);
    out[`${size.key}Height`] = Math.round((height || size.edge) * scale);
  }
  return out;
}
...
function build() {
  ...
  for (const [dishId, file] of Object.entries(TABLE)) {
    ...
    const record = {
      ...
      height: entry.height,
      file,
      // The sizes the pages display, generated by scripts/build-thumbs.js and
      // committed. `src` stays the original and is what the lightbox opens:
      // it is the one place a full size file is the right answer. A 92px tile
      // serving a 1280px original cost 7.40 MB to read Convoy's menu on a
      // phone, measured before this existed.
      ...derivatives(file, entry.width, entry.height),
    };

...
- Confirmed and usable: two years to build, machine imported from Japan, two noodle types, broth made every morning and driven to every SD location, previously bought frozen from a factory, and **noodles made daily (confirmed 2026-09-09, all locations including Maui)**.
- **Do not write:** "every bowl at every location" (rollout unconfirmed), any production time or clock (a mix, rest, sheet or cut hour: none is sourced), any flour type, hydration, water, pH, alkalinity, or aging claim (no source), the charcoal noodle (in development, not in production), "a few miles" (Crown Point to Plaza Bonita is ~17 miles).
...
|---|---|---|---|
| 1 | **Maui's noodles. STILL OPEN, evidence upgraded 2026-08-04.** Crown Point supplies SD. Maui is 2,500 miles away. **New:** Maui's live Toast menu labels **every ramen item "HM Noodles."** That is a much stronger signal than the `...HM-Noodles...Kihei.jpg` filename it supersedes, because it is Tajima's own operational system rather than an asset name, and it is applied consistently rather than once. **It still does not close this.** "HM" is presumed to expand to house-made, which is an inference, and the label says nothing about *mechanism*: whether Maui makes its own on site, receives shipments, or inherited the tag from the SD catalog. Any of the three changes what the site may claim. **Ask Amanda this exact question:** does Maui produce noodles on site, and if so on what equipment. **MATERIAL DEVELOPMENT 2026-09-09, AND THIS DECISION IS DELIBERATELY NOT RESOLVED HERE.** The client's cadence confirmation of that date explicitly covers **all locations including Maui**: house-made noodles, crafted daily. That is the closest thing to an answer this decision has ever had, and it is the question it was blocked on. It is left open on purpose because it does not settle **mechanism**, which is what this row actually asks: confirming that Maui serves house-made noodles made daily does not say whether Maui makes them on site, on what equipment, or receives them. "House-made" can be true of a group without being true of a kitchen. Resolving this needs someone to put the mechanism question to Amanda and get a sentence back. **Until then the rule is unchanged:** every "our noodles" claim stays scoped to San Diego, `/tajima-ramen-maui-hawaii/` keeps the Hawaii-scoped noodle band copy it has, and no Maui noodle-process photograph is used as evidence. | `/tajima-ramen-maui-hawaii/`, `/`, `/noodle-room/` | **Amanda, with the HM Noodles detail** |
| 2 | **Rollout status.** Is every SD location on house-made noodles? | any "every location" claim sitewide | Amanda (Interview Q14) |
| 3 | ~~**Noodle cadence.**~~ **RESOLVED 2026-09-09 by the client:** crafted daily is accurate at all locations including Maui. Five template cautions removed, two live claims on `/` confirmed, `site.json` `_sloganNote` has the record. Still open and separate: the production **clock**. | ~~`/noodle-room/`, slogan use~~ closed | ~~Amanda (Q13)~~ done |
| 4 | ~~**Carnitas.**~~ **RESOLVED 2026-08-04.** Confirmed active at **all seven** locations via live Toast menus. Shipping per the standing recommendation: listed plainly on `/menu/`, `feature: no`, nowhere else. No photo, no card, no callout, no hero. Now in `menu.json` with `locations` populated for all seven, the only row with real availability data. **This unblocks `/menu/` and its `Menu` schema.** ⚠️ It also contradicts two hard rules that still say the opposite: `DESIGN_SYSTEM.md` Banned content patterns (*"Carnitas Ramen does not appear on this site"*) and `voice-tone.md` Never write (*"Carnitas Ramen, or any fusion framing"*). See #21. | ~~`/menu/` and its schema~~ closed | ~~Sam~~ done |
...
| 12 | ~~**The per-location menus. We don't have them.**~~ **RESOLVED 2026-08-04.** All seven live Toast catalogs pulled and committed to `src/_data/toastMenus/` (379 items). `menu.json`'s `locations` column is now derived from them by exact item-name match through an explicit alias table. Result: 3 dishes at all seven, 4 at six, 2 at five, and the pattern matches what `CLIENT_FACTS.md` predicted only in part (see #22, #23). **This unblocks `/menu/`'s location switcher, every location page's menu section, and per-location `Menu` schema.** Prices are a separate problem, see #24. | ~~`/menu/`~~ closed | ~~Amanda / Marco~~ done |
...
| 20 | ~~**Sushi at East Village contradicts CLIENT_FACTS.md.**~~ **RESOLVED 2026-08-04: CLIENT_FACTS.md is wrong.** East Village's live Toast catalog carries a **Salmon Sushi Hand Roll ($6.50)** and a **Spicy Tuna Sushi Hand Roll ($6.50)**, listed under Appetizers rather than a sushi section of their own. Mercury is therefore NOT the only San Diego location with sushi. It remains the only one with a full sushi *program*: four sashimi plates and eleven specialty rolls against East Village's two hand rolls, so the distinction worth writing is scale, not presence. **CLIENT_FACTS.md needs correcting in two places** and the `/menu/` dietary key can now say something true instead of the placeholder it got on 2026-08-04.<br><br>**Update 2026-08-05: `CLIENT_FACTS.md` was corrected on 2026-08-04, but a published page still carried the false claim for a day.** `/tajima-convoy/` said Mercury has *"the only sushi in San Diego"* in its nearest-neighbour section. Correcting the source document did not correct the surface, and nothing searched the templates for the claim. **Now fixed**, per `CLIENT_FACTS.md`'s "write scale, not exclusivity": the line states the size of Mercury's list, scoped to Tajima, and claims sole possession of nothing.<br><br>Verified against the catalogs the same day: **Mercury** is the only San Diego room with a dedicated Sushi section (4 sashimi plates, 10 rolls, 14 items); **East Village** has 2 hand rolls under Appetizers; **Convoy** and **College Heights** carry poke but no sushi; **Crown Point** and **Plaza Bonita** have neither. Note `CLIENT_FACTS.md` says "eleven specialty rolls" where the live catalog has ten. Minor, unreconciled, do not print a roll count.<br><br>⚠️ **Still open:** `DESIGN_SYSTEM.md` Banned content patterns still reads *"No sushi in the brand voice. Mercury and Maui only"*, which is stale for the same reason (East Village has sushi). Same failure shape as #21. And the `/menu/` dietary key still holds its placeholder. | ~~`/tajima-convoy/`~~ fixed · `/tajima-east-village/`, `/menu/`, `DESIGN_SYSTEM.md` | Steve |
| 22 | **`menu.json` calls two dishes something Toast does not.** Not typos, and not auto-corrected, because renaming a menu item is a client decision. (1) **`Tajima White`** is called **`Tajima Ramen`** on all six San Diego catalogs and `Tajima WHITE Ramen` only on Maui. The San Diego menu has no item literally called Tajima White, and `/menu/` currently prints one. **Still open.** (2) ~~**`Miso Ramen`** is called **`Naruto Miso Ramen`** everywhere it appears, and the Toast description names a Naruto fishcake that `menu.json` does not mention.~~ **RESOLVED 2026-09-18: it was never a menu item.** Connor confirms Naruto Miso Ramen was a one-week Comic-Con special, which is why the August 4 Toast pull caught it at all six rooms while every printed menu and Connor's dish sheet omit it. The row is deleted from `menu.json`, not renamed, and excluded from the `roomMenus.js` Toast ingest so a future pull cannot reintroduce it. See `CLIENT_FACTS.md`. | `/menu/`, `Menu` schema, every location menu section | Steve / Amanda, Tajima White only |
| 23 | **Two `CLIENT_FACTS.md` menu characterisations are contradicted by the catalogs.** (1) It calls College Heights a *"pared-down ramen-bar menu, not the full menu"*. Its **ramen section is not pared down at all**: all nine San Diego ramens at Convoy's prices. What is reduced is everything around them, plus it has the largest beer program of the seven at 21 taps. (2) It groups **Crown Point** with Convoy, Mercury and East Village as running *"the full menu"*, but Crown Point's appetiser list is the shortest of those four (8 items against Convoy's 17) and it has no poke. Plaza Bonita's characterisation, by contrast, is confirmed exactly. Flagged, not resolved. | location page copy, `/menu/` | Amanda |
| 24 | ~~**`menu.json` prices are stale, and a single brand price may be the wrong model.**~~ **RESOLVED 2026-09-10 at client review, and the answer was neither of the options on the table.** The client asked for prices off the main menu page entirely. `/menu/` now prints none, the `price` field stays in `menu.json` as a record and is marked not-for-display, and the only prices on the site are on the per-location printable menus in `public/menus/`, which are the client's own current print and are per room by definition. Both findings in this row were right and are preserved in `menu.json` `_priceStatus`. | ~~`/menu/`~~ closed | ~~Amanda~~ done |
| 25 | **First real happy hour pricing has arrived, for one location.** Plaza Bonita's catalog carries a priced **Happy Hours** section: HH Traditional Tonkotsu Ramen $11.00, HH Chicken Ramen $12.00, HH Karaage $5.50, HH Pork Gyoza $4.50, HH Onigiri $6.00, plus HH drinks. This is the first happy hour data on the project from a source rather than a design concept, and `src/_data/happyHour.json` currently holds no times or prices at all after the concept values were pulled on 2026-08-03. **It does not refill that file**: it is one location of six and carries no times. But it is a real crack in that blocker and the other five catalogs may have equivalents nobody has pulled yet. | `/happy-hour/` | Amanda |
| 26 | **36 photos are still over the 300 KB image budget.** From the batches that landed before the GBP ingest, not from it: mostly `tajima-ramen-*-kihei` and `tajima-ramen-convoy-*`, ranging to 870 KB. They were deliberately left alone on 2026-08-04. Re-encoding costs a lossy generation, and nobody has decided the target dimensions or which of them will ever be used, so spending that generation now buys nothing. Do it as one pass **after** the dedup pass and after placement decisions, when the surviving set is known. `node scripts/build-photo-manifest.js` reports sizes. | Core Web Vitals, LCP | Steve |
| 27 | ~~**`inspo/` is untracked and has stayed that way.**~~ **CLOSED 2026-08-05 by deletion, on Steve's call.** The directory is gone and all 22 files that referenced it were rewritten to name the concept without a path (`the tajima-home concept`, not `inspo/tajima-home.html`). ⚠️ **Closed with a real loss, recorded rather than papered over.** The decision was taken on the basis that everything usable had been built into real pages, which is true of the *output*. It is not true of the *provenance*: a check of `cdn.circulationstudio.com/tajima-temp/design-concepts/` on the day of deletion found **only two of the eight concepts hosted there**, and the missing six include **`tajima-home.html`, which `DESIGN_SYSTEM.md` names the canonical token source and which eleven CSS files were ported from.** That doc claimed all three of its listed concepts were hosted; it was wrong about `tajima-home.html` and has been corrected. **The tokens are not at risk** (`src/css/base/theme.css` is committed and is now the operative source rather than a port). What is gone is the visual record of how each component was originally drawn. **Follow-up: re-upload the six missing concepts to the CDN from the design source.** | ~~reproducibility of every ported component~~ closed, with the follow-up above outstanding | Steve |
| 30 | 🚨 **PRE-LAUNCH BLOCKER. `/tajima-convoy/`'s hero image is hotlinked from the WordPress site this project replaces, and there is no replacement anywhere.** Decommissioning WordPress breaks the hero on the highest-traffic page on the site. **This must be sequenced before the cutover, not discovered after it.** Full detail below, including **a drafted message to Amanda** that bundles this with three other things the same visit can close: a photograph of Sam (`/about/` has none of the founder), the commissary (`CLIENT_FACTS.md` #15), and reading the Crown Point door number (#28). | `/tajima-convoy/` (9,704 clicks), and the WordPress decommission date | **Steve / Amanda, before cutover** |
| 28 | **Crown Point's best exterior photograph shows the street number 3784. We publish 3782.** Found 2026-08-05 while curating `roomPhotos.js`. `tajima-crown-point-exterior-night-signage-neon-ramen.webp` is the strongest wide exterior in the set and the wall number beside the door plainly reads **3784**. Every record we hold says **3782 Ingraham Street**: `locations.json`, `CLIENT_FACTS.md`, the NAP table above, and Toast's own page for the store. Most likely the neighbouring unit's number, or a building number against a suite number, but **nobody has checked.** The photo is held out of the gallery rather than published, because putting a visible contradiction of our own NAP on the page is exactly the failure the citation cleanup exists to stop, and Convoy's audit already scored 25/100. The neon detail frame carries the exterior instead and shows no number. **Resolve by looking at the door**, then either publish the photo or correct the address. **Folded into the #30 photography ask:** the commissary is in this building, so whoever goes for that shoot can read the number off the door at no extra cost. | `/tajima-crown-point/` photography, and possibly Crown Point NAP everywhere | Steve / Connor |
| 29 | **Four of the seven rooms have no usable photography at all.** Quantified 2026-08-05 during placement. College Heights has 24 frames and Crown Point 6, and those are the only two rooms that can show themselves. **Mercury** has exactly two room frames, at 612x284 and 1000x750, too small to place. **East Village's** single photograph is a plate of shishito peppers, not a room. **Plaza Bonita** and **Maui** have no room photography. **Convoy has 60 photographs and every one is food.** This is the concrete, costed version of `CLIENT_FACTS.md` open question #15. **`/about/` is the sharpest instance of this gap and is worth naming separately.** It is a founder story with **no photograph of the founder and none of the founding room**, because neither exists in the manifest. It currently carries College Heights and Crown Point interiors, captioned by location so neither can be misread as Convoy. A portrait of Sam is folded into the #30 photography ask, since it is the same visit. ⚠️ **The Convoy storefront has been split out into #30 and is a pre-launch blocker. Do not close #29 and #30 together:** everything in #29 degrades gracefully (the gallery component renders nothing for a location with no curated set), whereas #30 breaks a live page on a date somebody else controls. | five location pages | Steve / Amanda |
| 31 | **In brand mode, `--font-cond`, `--font-body` and `--font-ui` all lead with Calps, so section titles, body copy and UI labels are the same typeface.** Traced 2026-08-05 through the built bundle (`_site/assets/main.*.css`), not just the declaration. `theme.css` sets `--font-cond-brand: "Calps", "Barlow Condensed", sans-serif`, `--font-body-brand: "Calps", "Barlow", sans-serif`, `--font-ui-brand: "Calps", "Barlow", sans-serif`. Calps leads all three, and it loads (HTTP 200, 109,948 bytes), so **the fallbacks never render and the token named for a condensed face is not condensed.** Every `.sect__title` (H2) on every location page, every `.chapter__title` (H2) on `/about/`, and `.diff__title` / `.next-card__title` (H3) render in the same face as the paragraph beneath them, separated only by size, weight and tracking. **This is not a bug. It is what `theme.css` declares, and the build is faithfully doing it.** The problem is that nothing records it: `DESIGN_SYSTEM.md`'s font table describes Calps only as *"Body. Clean legible sans. Default for all body copy and UI"*, which is now incomplete, and reading the doc alone would leave you expecting a hierarchy the site does not have. **In `editorial` mode the distinction is real** (Barlow Condensed against Barlow), so this is specific to the `brand` set that shipped. **Three ways out:** (a) accept it and correct the font table, (b) point `--font-cond-brand` at Barlow Condensed while leaving body and UI on Calps, a one-line change that restores the hierarchy, or (c) flip `site.json` `theme.fonts` to `editorial`, which reverses #15 and is the largest change. ⚠️ **Distinct from #15 and #16, do not close them together.** #15 covers Bright Sunshine Caps being the default `h1` against the doc's restriction, and only `--font-display` classes (`.page-hero__title`, `.menu-hero__title`, `.menu-section__title`, `.noodle-band__slogan`) are affected by it; #16 covers licensing and self-hosting for both faces. This is the third, separate issue and it touches neither. **Related finding worth noting against #15:** because most H2s resolve through `--font-cond`, Bright Sunshine Caps is **not** in fact the default `h2` sitewide, which is what #15 assumes. It is the H1, plus two specific H2 classes. | `DESIGN_SYSTEM.md` accuracy, and the typographic hierarchy on every page carrying a `.sect__title` | Steve / Ana |
| 21 | ~~**`DESIGN_SYSTEM.md` and `voice-tone.md` still ban Carnitas Ramen outright.**~~ **CLOSED 2026-09-10.** Both documents are corrected, and by a bigger change than this row anticipated: at client review the dish was confirmed **permanent, with a photograph**, which supersedes the 2026-08-04 'listed, never featured' position as well as the original ban. `/menu/` now carries a Carnitas photograph, `feature` stays false so it is not promoted into the location pages, and `scripts/build-photo-manifest.js` no longer excludes Carnitas frames. The fusion-framing ban is untouched and absolute. | ~~`/menu/`, both hard-rule docs~~ closed | ~~Steve~~ done |
| 32 | **A template class with no matching CSS rule is now a recurring failure shape on this project. Make grepping for it a standing habit.** Third instance found 2026-08-05, and the first two are already sitting in this table as #18 and #19. This one: `happy-hour.css` defined `.loc-close__links`, which **no template uses**, while the templates used `.loc-close`, `.loc-close__inner`, `.loc-close__title` and `.loc-close__sub`, **none of which had a single rule anywhere in the stylesheet.** The closing "Pick a room." section therefore rendered with no gutter, no max-width, no centring and no type styling, flush to the viewport edge, **on three pages** (`/happy-hour/`, `/order-online/`, `/menu/vegan-ramen/`). Cause was a BEM rename in the templates that never reached the CSS. **The shared property across all three instances is that nothing errors:** the build is green, the markup is valid, the CSS is valid, and the only symptom is a page that looks wrong to a human who happens to load it. #18 is a renamed data key rendering an empty loop; #19 is `where()` returning `[]` for a misspelled key; this is a renamed class matching no rule. Same shape, three different layers. **Proposed standing check, cheap enough to run on every build:** extract every `class="..."` token from `src/**/*.njk`, extract every class selector from `src/css/**/*.css`, and report both directions. Classes in templates with no rule are the bug that just shipped; rules with no template are dead CSS and worth knowing too (`.loc-close__links` was both a symptom and a leftover). Tailwind utilities and state classes need an allowlist, so it wants to start as a warning rather than a hard failure. | silent visual breakage on any page, and it has already happened three times | Steve |
| 33 | **Band order on the menu: do appetisers come before ramen?** The menu module orders its bands `izakaya, ramen, rice, sushi, monthly special, dessert, kids, combo`, so a reader meets the appetisers first. Measured at 390px on the built pages: **Convoy puts seventeen appetisers ahead of its first bowl, and ramen starts 2,290px in, 2.7 screens.** Mercury 2.4 screens, Plaza Bonita and Maui 2.1. **The case for moving ramen first:** this is a ramen house, the North Star is that the proof is in the bowl, every location page targets a ramen keyword, and half of `/menu/`'s core set is ramen. It costs one line in `menuConcepts.json` and removes about two and a half screens of scrolling before the thing people came for, with no new interface. **The case against:** the client's own 2026-07 printed menus lead with appetisers, and so does the approved A2 reference. Matching the printed menu is a real reason, and this is a decision about how Tajima presents its own food rather than a build preference. **Not decided here on purpose.** If it changes, run the adjacent-tone guard in `eleventy.config.js` first: the reorder changes which bands touch, and it throws when two neighbours share a tone. | every location page's menu section | **Amanda** |
| 34 | **`menu.json`'s `locations` arrays are inert, and that is a trap rather than a tidy-up.** Since the Toast rebuild on 2026-09-16 no page reads them: a location page renders its own catalog through `roomMenus.js` and `/menu/` computes its core set from the same rows through `coreMenu.js`. **The specific risk is silent:** somebody edits a `locations` array to add or remove a dish from a room, the build succeeds, the JSON stays valid, and the page does not change. Nothing on the surface says which fields are live. The file is HALF live, which is the dangerous shape: `description` and `dietary` ARE still read, per dish, joined through `menuAliases.json`, so edits to those do work. To change where a dish is served you change the Toast catalog or the alias table; to change how it reads you change `menu.json`. Recorded in that file's `_locationsAreNoLongerRead`. **Not resolved yet on purpose:** deleting the arrays is a 73-row diff in the same pass that rebuilt the menu layer, and the safe order is to let the Toast-sourced pages sit in front of the client first. | nothing today, and a wasted afternoon for whoever edits one | **Steve**, after client review |
| 18 | **Guard the five must-never-be-empty template loops.** `nonEmpty()` exists in `eleventy.config.js` and is applied to exactly one loop, `nav.primary`, because that is the one that actually broke. Renaming a data key without updating its template renders nothing, reports nothing, and builds green: every page shipped with an empty `<nav>` before the guard existed. Five loops carry the same exposure and would silently ship a broken page: `nav.footer` (footer on all 17 pages), `locations.items` (mega menu, home router, `/order-online/`, 4 uses), `menu.items` (home teaser, Convoy gallery, 2 uses), `site.stats` (home stat bar), `site.footer.spec` (footer spec strip). Each is a one-line change, `\| nonEmpty("<key>")`. **Do NOT blanket-apply it.** `hh.windows`, `hh.drinks`, and `hh.food` are legitimately empty right now, because the happy hour times were deliberately pulled, and guarding them would fail the build on the one thing we emptied on purpose. | silent empty renders on 17 pages | Steve |
| 19 | **`where()` can silently return nothing, and no guard covers it.** Separate from #18 and a different class of fix. `#18` guards call sites; this is about guarding the filter itself. `where(items, key, value)` in `eleventy.config.js` returns `[]` when `key` is misspelled, renamed, or absent from the data shape, exactly like the Nunjucks `selectattr` bug it was written to replace (`TEAM_NOTE-nunjucks-selectattr.md`). Five loops consume its output: `sanDiego` and `hawaii` in `locations.njk`, `ramen`, `izakaya`, and `dessert` in `menu.njk`. A typo in any of those `where()` calls empties a whole page section with no error. **The fix is in the filter, not the call sites:** throw when `key` is present on none of the items, which is a data-shape error, while still allowing a legitimately empty result when the key exists but nothing matches the value. Guarding call sites instead would need a `nonEmpty` on every one and would still miss the next `where()` anyone adds. **This is the most likely candidate to become the next silent-failure bug**, because the pattern is already established, already used five times, and reads as correct. | every section rendered through `where()` | Steve |
| 17 | **`Order Online` is unreachable from `/noodle-room/`.** The header now carries Order Online as its single red CTA on all sixteen main-family pages. `/noodle-room/` runs its own header (`layouts/noodle-room.njk`: back link, editorial mark, Menu / About / Locations) and has neither Order Online nor the Locations mega menu. That header is deliberately separate, `DESIGN_SYSTEM.md`: *"a sub-experience inside the site, not a page in the main template family. Do not normalize it into Convoy Red."* So this is a genuine conflict between two correct rules, not an oversight. `/order-online/` is the third-biggest page on the site (5,648 clicks) and the Noodle Room is the P0 growth priority, so the page most likely to convert belief into an order is the one page that cannot link to ordering. **Three options:** leave it (the footer still links Order Online from every page, including this one); add Order Online to the Noodle Room header as a plain link in the sub-experience's own cream-and-serif language, not the Convoy Red pill; or normalize the header and accept the `DESIGN_SYSTEM.md` conflict. **Recommendation: the middle one.** Deferred from build night deliberately, this is a daylight decision. | `/noodle-room/` conversion path | Steve / Ana |
...
## On #30, the Convoy hero (PRE-LAUNCH BLOCKER)

**This is not the same problem as #29 and must not be closed alongside it.** #29 is an inventory gap: five location pages would be better with photography they do not have, and they degrade gracefully without it because the gallery component renders nothing when a location has no curated set. #30 is a **dependency with a deadline attached.** The page does not degrade. It breaks, on a date somebody else controls.

### What it is

`src/tajima-convoy.njk` renders the page's hero photograph straight off the live WordPress site:

```
https://tajimaramen.com/wp-content/uploads/2022/06/Tajima-Convoy-3-2.jpg
```

**One line, one page, one image.** Verified 2026-08-05: this is the only `wp-content` reference in the entire source tree and the only one in the built output. It is contained, which is the good news and also why it is easy to miss.

### Why it is a blocker rather than a to-do

1. **It is on the highest-traffic page on the site.** `/tajima-convoy/`: 9,704 clicks, 678,630 impressions, position 2.83. It also owns `best ramen san diego`, not the home page.
2. **The failure is a broken image on the flagship location page**, appearing the moment WordPress goes away, with nothing in the build to catch it. Eleventy will keep building green: it is an absolute URL to another host, so no template error, no missing-file warning, no failing test.
3. **Migration checklists catch links and redirects. They do not usually catch a hotlinked image**, because nothing about it looks like a link. This is precisely the class of thing found after cutover rather than before.
4. **There is no replacement.** Verified against the manifest 2026-08-05: **zero Convoy exterior, storefront, or interior frames exist among all 205 photographs.** All 60 Convoy files are food and drink. No filename in the whole manifest matches `exterior`, `storefront`, `interior`, `dining` or `facade` for Convoy. Swapping the `src` to a local file is not an available option, which is what separates this from every other image on the site.

### Resolve it one of three ways, before the decommission date

| | What | Cost | Result |
|---|---|---|---|
| **A** | **Shoot the Convoy storefront.** Preferred. It is the founding room and the site has no picture of it. | A photographer and an hour. | Fixes it properly, and fills the most conspicuous hole in the brand's own asset library. |
| **B** | **Get the original file from Tajima and commit it.** The image already exists; it is sitting in a WordPress media library. Ask Amanda to export it, add it to `public/images/photo/`, regenerate the manifest, repoint the `src`. | Minutes, once someone sends the file. | Removes the dependency. Keeps a 2022 photograph, which is worse than A but is what is on the site today. |
| **C** | **Remove the hero figure.** `.page-hero--split` becomes a single-column hero, which the layout already supports. | One edit. | Safe, and worse: the flagship location page ships with no photograph of the room. Use only if the cutover arrives with A and B unresolved. |

**B is the minimum acceptable action and it should happen now rather than at cutover**, because it costs almost nothing and it converts a deadline into a non-event. A can then follow on its own schedule.

### The ask to Amanda, drafted 2026-08-05

**Scope widened deliberately.** #30 alone only needs the Convoy storefront. But the same visit can close three other things, and a second visit will not be easier to arrange than the first. Send this as one ask, not three.

What the one visit resolves:

| | Gap | Currently |
|---|---|---|
| **Convoy storefront** | #30, the blocker | Hotlinked off the WordPress site being decommissioned. No replacement exists. |
| **Sam** | `/about/` has no photograph of the founder | A founder-story page with no picture of the founder, and none of the founding room either. Sharper version of #29 on the page where it is most conspicuous. |
| **The commissary** | `CLIENT_FACTS.md` open question #15 | The documentary Noodle Room shoot does not exist. `/noodle-room/` runs on temp-CDN images. P0 growth priority. |
| **The Crown Point door number** | #28 | Costs nothing: the commissary is in the Crown Point building, so whoever goes can read the number off the door. |

**Draft message:**

> Hi Amanda,
>
> We need some photography before we can retire the current WordPress site, and it is one visit if we can line it up.
>
> **The one we actually need: the Convoy storefront and exterior.** The new site is currently loading its Convoy hero image straight off the old WordPress site. The moment that site comes down, the main image on our highest-traffic page breaks, and we have no replacement: of the 205 photographs we hold, all 60 of the Convoy ones are food. There is no picture of the building.
>
> Two options there. If a photographer can get to Convoy, that is the right fix and the picture will be current. If that is going to take a while, the quick version is to export the existing image from the WordPress media library and send it to us (it is `Tajima-Convoy-3-2.jpg`, from June 2022). That removes the dependency today and we can replace it with something better later. Either way we need it before the old site is switched off, not after.
>
> **Two more while someone is out with a camera, both worth having:**
>
> **Sam, if he is willing and available.** The About page tells his story from Amagasaki to Convoy and there is not a single photograph of him on it. A portrait in one of the rooms would do; it does not need to be a production.
>
> **The commissary in Crown Point, if it is convenient.** The house-made noodles and the daily broth are the whole argument the new site makes, and we have no photographs of either being made. Anything real is better than what we have, which is nothing. Two things to keep in mind if this happens: we are deliberately not publishing the commissary's street address anywhere (it is a working kitchen, not somewhere we want people turning up), so nothing that pins the location, and please no shots framed as though they were taken at another location.
>
> **One small favour if anyone is at Crown Point anyway:** could they read the street number off the door and tell us what it says? We publish 3782 Ingraham Street everywhere, but our best exterior photograph of that building clearly shows 3784 on the wall. It is probably the unit next door, but we are holding that photograph back until someone confirms it, and we would rather know than guess.
>
> No rush on the last three. The Convoy one has a deadline attached to it.
>
> Thanks,
> Steve

**Still outstanding with Amanda and NOT in this message,** because they are questions rather than a photography request and bundling them would bury the deadline: confirmed hours for all seven (#9), Maui's noodle production (#1), the College Heights menu characterisation (#23), happy hour windows and prices (#25), whether there is an events programme (#13), whether Tajima does catering (#14), and the College Heights manga wall (`CLIENT_FACTS.md`). Send those as their own message.

### One more defect on the same line, fix it in the same pass

The hero carries `loading="lazy"`. **A hero image is the LCP element and must never be lazy-loaded**; `CLAUDE.md` Performance Standards require `loading="eager"` plus `fetchpriority="high"` plus a preload. `layouts/base.njk` now has a `heroImage` front-matter field that emits the preload (added for `/tajima-college-heights/`, which does this correctly). Whoever resolves the source should fix the loading attributes at the same time. Doing it before the image is local would only make the site fetch a third-party image sooner, so it waits for A or B.

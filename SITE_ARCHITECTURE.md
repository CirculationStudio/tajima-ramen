...
| 24 | **`menu.json` prices are stale, and a single brand price may be the wrong model.** Nine of thirteen are below live Convoy: every ramen is $0.50 under except `Miso Ramen`, which is $0.50 **over**. The four appetiser and dessert prices match Convoy exactly. More structurally, price varies by location: **Plaza Bonita runs $2.00 below** the other San Diego rooms and **Maui runs $2.50 to $3.50 above** them, so no single figure is correct for all seven. Decide whether `/menu/` prints Convoy's prices, a range, or none, before any `offers` are emitted. No `offers` are emitted today. | `/menu/`, `Menu` schema `offers` | Steve / Amanda |
...
| 21 | **`DESIGN_SYSTEM.md` and `voice-tone.md` still ban Carnitas Ramen outright.** #4 closed on 2026-08-04 with a decision to list it plainly on `/menu/`. Two hard rules now contradict that decision and will cause the next agent to delete it as a violation: `DESIGN_SYSTEM.md` Banned content patterns, *"**Carnitas Ramen does not appear on this site**, in copy, photography, or menu hero content"*, and `voice-tone.md` Never write, *"Carnitas Ramen, or any fusion framing of the brand"*. **The intent behind both is still correct and must survive the edit:** no photo, no hero, no card, no callout, no fusion framing anywhere. Only the absolute *"does not appear on this site"* needs to become *"appears once, as a plain line on `/menu/`, and nowhere else"*. Same failure shape as the Convoy Suite A correction: a first-priority doc left stating something the build no longer does. | the next agent silently deleting a shipped menu item | Steve |

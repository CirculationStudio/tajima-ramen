...
| 22 | **`menu.json` calls two dishes something Toast does not.** Not typos, and not auto-corrected, because renaming a menu item is a client decision. (1) **`Tajima White`** is called **`Tajima Ramen`** on all six San Diego catalogs and `Tajima WHITE Ramen` only on Maui. The San Diego menu has no item literally called Tajima White, and `/menu/` currently prints one. (2) **`Miso Ramen`** is called **`Naruto Miso Ramen`** everywhere it appears, and the Toast description names a Naruto fishcake that `menu.json` does not mention. Decide which name the site uses, then make `menu.json` match. | `/menu/`, `Menu` schema, every location menu section | Steve / Amanda |

...
| 20 | **Sushi at East Village contradicts CLIENT_FACTS.md.** `CLIENT_FACTS.md` states in two places that Mercury is the only San Diego location with sushi (Location 2: *"The only location with sushi"*; The menu: *"Mercury and Maui are the only locations with sushi"*). **East Village's live Toast menu carries a Sushi Hand Roll section** (observed 2026-08-04). One of the two is wrong. Until Amanda confirms which: **no "no sushi here" language ships on `/tajima-east-village/`**, and the exclusivity claim is pulled from `/menu/`'s dietary key, which was rendering *"Sushi: Mercury and Maui only"* to the public. Note this is not only a copy problem, it is a **menu-data** problem: if East Village sells sushi, its `locations` availability differs from what anyone assumed, and it may be true of other locations nobody has checked. Toast is the arbiter. | `/tajima-east-village/`, `/menu/` dietary key, `menu.json` availability | **Amanda** |

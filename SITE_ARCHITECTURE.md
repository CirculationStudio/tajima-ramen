...
| 27 | **`inspo/` is untracked and has stayed that way.** The eight signed design concepts the whole build was ported from. Everything referencing them (`DESIGN_SYSTEM.md`, and the porting notes in most component and page CSS files) points at paths that exist on one machine. Either commit them as the design reference of record, or move them somewhere durable and update the references. Leaving it is the option that quietly rots. | reproducibility of every ported component | Steve |

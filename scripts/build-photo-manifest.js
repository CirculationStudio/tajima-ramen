...
// ---------------------------------------------------------------------------
//
// PLACEHOLDER FIELDS, added 2026-09-09. `placeholder`, `placeholderSource` and
// `placeholderNote` are hand-authored like the rest: a filename cannot tell
// you whether a photograph is a stand-in. They are in this list so that
// running the generator cannot quietly unmark a stock image, which would take
// the build guard in src/_data/roomPhotos.js down with it. See the
// _placeholderRule key in photos.json for what the fields mean.
const HAND_FIELDS = [
  "locationStatus",
  "duplicateGroup",
  "duplicateNote",
...
for (const file of files) {
  ...
  if (ambiguous) {
    // AN AMBIGUOUS DISH TOKEN IS A REASON NOT TO ASSIGN A DISH. It is not a
    // reason to forget the file exists, and until 2026-09-15 this branch did
    // exactly that: it `continue`d before `manifest[file] = entry`, so the
    // whole record went with it, alt text included.
    //
    // It is the same fault the commissary set hit and the same fix. See the
    // note further down about those five. It stayed hidden here because almost
    // nothing was ambiguous while menu.json held fourteen dishes; expanding it
    // to the full catalog on 2026-09-15 made nineteen filenames ambiguous at
    // once and took nine hand-written alt strings out with them. Caught by
    // npm run test:photos, which is why that test exists.
    //
    // The file stays in `photos` with dish: null, and it is STILL listed in
    // UNMAPPED so a human sees it and rules. src/_data/dishPhotos.js is where
    // that ruling gets written down, and it throws if a dish it maps is not in
    // this manifest, so the two cannot drift apart.
    entry.dish = null;
    unmapped.push({
      file,
      reason: `Filename matches more than one dish on menu.json (${ambiguous.join(", ")}). Not guessed, and dish is left null. A human has to say which, in src/_data/dishPhotos.js, or confirm it is a dish that is not on menu.json at all. The entry is kept so its alt text and dimensions survive the ruling.`,
    });
    manifest[file] = entry;
    continue;
  }
...
const output = {
  _note: "Generated by scripts/build-photo-manifest.js. Do not hand-edit the mapping; fix the filename or the script's token list and regenerate. Alt text IS hand-edited, and as of 2026-08-05 a regenerate PRESERVES it rather than overwriting it: the script only writes the [DRAFT, NEEDS REVIEW] placeholder for entries that do not already have real alt. The same applies to locationStatus, duplicateGroup, duplicateNote, proposedUseNote and proposedUseLocked, and to proposedUse itself where proposedUseLocked is true. Run `npm run test:photos` to prove a regenerate is non-destructive. Placeholder fields (placeholder, placeholderSource, placeholderNote) are hand-authored and preserved the same way; see _placeholderRule.",
  _rule: "The filename is the source of truth for what a photo shows. The directory a file arrived in is never read.\n\nA photo of a PLACE reaches a location page only if its filename names that location. That binds type room, exterior and interior. A photograph of a dining room is evidence about that dining room and nowhere else, and putting College Heights' counter on the Crown Point page would be a claim about a room nobody photographed.\n\nIT DOES NOT BIND type dish, AMENDED 2026-09-15. A bowl of Tajima Red is the same bowl in every room: same commissary broth, same Crown Point noodles, same build. Scoping food by filename was not protecting anything, it was just wrong, and it was expensively wrong. Every dish photograph in this manifest is named for Convoy or Kihei, so the rule made five of the seven rooms look unphotographed while Connor's 2026-09-15 dish sheet puts real coverage at 62 to 100 percent. The menu module rendered placeholder tiles on hundreds of cards that had a photograph available.\n\nWHAT REPLACES IT FOR FOOD is not laxity, it is a different and stricter test: a dish photograph is placed only through the hand-verified table in src/_data/dishPhotos.js, where every entry has been opened and looked at. The filename is still never trusted on its own. See the note at the top of that file for what trusting it cost.\n\nTHE EXCEPTION TO THE EXCEPTION, NARROWED 2026-09-16. This used to say that house-named rolls do not travel and to imply that standard constructions do. That split does not hold, and a photograph shipped on the strength of it.\n\nWHAT WENT WRONG. california-roll carried a KIHEI photograph on a dish menu.json lists at MERCURY ONLY, placed on the reasoning that a California Roll is a standard construction and therefore travels. The two Toast catalogs disagree: Maui's is crab mix, avocado and cucumber topped with tobiko; Mercury's is kanikama, avocado and cucumber. Description similarity 0.18, and kanikama is not crab. The name was standard and the dish was not.\n\nTHE TEST IS NOW THE CATALOG, NOT THE NAME. A dish photograph travels between rooms only where those rooms' Toast descriptions agree on the construction. Where they differ materially it stays scoped to the room it was shot in, whether the name sounds house-invented or generic. Nobody has to judge which dishes are 'standard' any more, which is good, because that judgement is what failed. src/_data/menuAliases.json records the comparison for all 106 Toast rows and names the five that share a name with a San Diego dish and are not that dish.\n\nThe house bowls still travel, and that is the whole point of the amendment above: Tajima Red is the same build on the same broth and the same noodles in every room, and the catalogs say so.",
  _ruleHistory: "The rule above read, in full, until 2026-09-15: \"The filename is the source of truth for what a photo shows. The directory a file arrived in is never read. A photo reaches a location page only if its filename names that location.\" It was written for room photography and applied to everything, which is the whole error. Recorded rather than overwritten so the next person can see that the narrowing was deliberate and what it was narrowed for.",
  _placeholderRule: "EVERY PHOTOGRAPH ON THIS SITE IS TAJIMA'S OWN. A stand-in is allowed only for a subject no Tajima photograph could ever cover (raw ingredients, dough texture, unbranded noodles, steam, wood or ceramic), never for a room, a storefront, or a plated bowl, and it is never presented as Tajima. Registering one is three hand-authored fields on its entry: placeholder: true, placeholderSource (the URL it came from), and placeholderNote (which real photograph replaces it and who is shooting it). All three are required and a partial registration fails too. src/_data/roomPhotos.js sweeps this whole manifest at module load and THROWS, which fails npm run build, so a placeholder cannot ship by being forgotten. TAJIMA_ALLOW_PLACEHOLDERS=1 npm run build is a client-preview escape hatch only: Cloudflare does not set it, so a placeholder still fails the deploy.",
